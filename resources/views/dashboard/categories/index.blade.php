...
<div class="max-w-4xl mx-auto space-y-6" dir="rtl">
    <div class="flex items-center justify-between">
        ...
        <span class="text-sm font-medium" style="color:#8a92a6">{{ ($categories ?? collect())->count() }} فئة</span>
    </div>
    <div class="surface-card rounded-2xl border p-5" style="border-color:#232530">
        <form action="{{ route('dashboard.categories.store') }}" method="POST" class="flex gap-3">
            @csrf
            <div class="flex-1">
                <input type="text" name="name" required class="w-full rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-1 focus:ring-accent-blue" style="background:#1a1c25;border:1px solid #232530;color:#fff" placeholder="اسم الفئة الجديدة...">
                @error('name') <span class="text-sm mt-1 block" style="color:#f87171">{{ $message }}</span> @enderror
            </div>
            <button type="submit" class="px-6 py-2.5 rounded-xl text-sm font-bold text-white transition-all hover:scale-[1.02]" style="background:linear-gradient(135deg,#4f8cff,#a78bfa)">
            ...
            <table class="w-full text-sm">
                <thead>
                    <tr style="background:rgba(255,255,255,0.03)">
                        <th class="text-right px-5 py-3 font-semibold text-xs" style="color:#8a92a6">#</th>
                        <th class="text-right px-5 py-3 font-semibold text-xs" style="color:#8a92a6">اسم الفئة</th>
                        <th class="text-right px-5 py-3 font-semibold text-xs" style="color:#8a92a6">عدد المنتجات</th>
                        <th class="text-right px-5 py-3 font-semibold text-xs" style="color:#8a92a6">تاريخ الإنشاء</th>
                ...
                <tbody class="divide-y" style="border-color:#232530">
                    @forelse($categories ?? [] as $category)
                        <tr class="transition-colors hover:bg-white/[0.02]" x-data="{ editing: false }">
                            <td class="px-5 py-3" style="color:#8a92a6">{{ $category->id }}</td>
                            <td class="px-5 py-3">
                                ...
                                <div x-show="editing" x-cloak>
                                    <form action="{{ route('dashboard.categories.update', $category->id) }}" method="POST" class="flex gap-2">
                                        @csrf @method('PUT')
                                        <input type="text" name="name" value="{{ $category->name }}" required class="rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:ring-1 focus:ring-accent-blue" style="background:#1a1c25;border:1px solid #4f8cff;color:#fff">
                            ...
                            <td class="px-5 py-3">
                                <span class="px-2.5 py-1 rounded-full text-xs font-medium" style="background:rgba(255,255,255,0.06);color:#8a92a6">{{ $category->products_count ?? $category->products->count() ?? 0 }} منتج</span>
                            </td>
                            <td class="px-5 py-3" style="color:#8a92a6">{{ $category->created_at->format('d/m/Y') }}</td>
                            <td class="px-5 py-3">
                    ...
                    @empty
                        <tr><td colspan="5" class="px-5 py-10 text-center" style="color:#555a6e">لا توجد فئات بعد</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
