...
<div class="space-y-6" dir="rtl" x-data="{ showModal: false }">
    ...
    <div class="stat-card overflow-hidden">
        ...
        @if($expenses->hasPages())<div class="px-4 py-3 border-t" style="border-color:var(--divider)">{{ $expenses->links() }}</div>@endif
    </div>
        ...
        <div @click.away="showModal = false" x-show="showModal" x-transition class="modal-light rounded-2xl shadow-2xl w-full max-w-lg" x-cloak>
            <div class="p-6">
                ...
                <form action="{{ route('dashboard.warehouse.storeExpense') }}" method="POST" class="space-y-4">
                    @csrf
                    <div>
                        <label class="block text-sm font-bold mb-1.5" style="color:#374151">نوع المصروف *</label>
                        <select name="type" required class="w-full rounded-xl px-4 py-3 text-sm font-medium" style="background:#f3f4f6;border:1px solid #d1d5db;color:#111827">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label class="block text-sm font-bold mb-1.5" style="color:#374151">المبلغ (د.ج) *</label>
                            <input type="number" name="amount" min="0" required class="w-full rounded-xl px-4 py-3 text-sm font-medium" style="background:#f3f4f6;border:1px solid #d1d5db;color:#111827" placeholder="0.00">
                        </div>
                        <div>
                            <label class="block text-sm font-bold mb-1.5" style="color:#374151">التاريخ *</label>
                            <input type="date" name="date" value="{{ date('Y-m-d') }}" required class="w-full rounded-xl px-4 py-3 text-sm font-medium" style="background:#f3f4f6;border:1px solid #d1d5db;color:#111827">
                        </div>
                    </div>
                    <div>
                        <label class="block text-sm font-bold mb-1.5" style="color:#374151">مرتبط بمنتج</label>
                        <select name="product_id" class="w-full rounded-xl px-4 py-3 text-sm font-medium" style="background:#f3f4f6;border:1px solid #d1d5db;color:#111827">
                    ...
                    <div>
                        <label class="block text-sm font-bold mb-1.5" style="color:#374151">الملاحظات</label>
                        <textarea name="description" rows="2" class="w-full rounded-xl px-4 py-3 text-sm font-medium" style="background:#f3f4f6;border:1px solid #d1d5db;color:#111827" placeholder="مثال: شحن طردين للبليدة"></textarea>
                    </div>
                    <div class="flex items-center gap-3 pt-2">
                        <button type="submit" class="flex-1 py-3 rounded-xl text-sm font-extrabold" style="background:#111827;color:#ffffff">تسجيل المصروف</button>
                        <button type="button" @click="showModal = false" class="px-6 py-3 rounded-xl text-sm font-bold" style="background:#f3f4f6;color:#374151">إلغاء</button>
                    </div>
                </form>
            </div>
        </div>
