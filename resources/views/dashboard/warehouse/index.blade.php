...
<div class="space-y-6" dir="rtl" x-data="{ activeTab: 'financial', showModal: false, showCostModal: false, selectedProduct: null }">
    ...
    <div class="flex items-center justify-between flex-wrap gap-3">
        ...
        <div class="flex items-center gap-2 flex-wrap">
            @foreach(['all'=>'الكل','today'=>'اليوم','yesterday'=>'أمس','7days'=>'7 أيام','30days'=>'30 يوم'] as $key => $label)
                <a href="?period={{ $key }}" class="px-3 py-1.5 rounded-xl text-xs font-bold transition-all" style="{{ request('period','all') === $key ? 'background:#111827;color:#ffffff' : 'background:var(--input-bg);color:var(--text-secondary)' }}">{{ $label }}</a>
            @endforeach
        </div>
    </div>

    {{-- ═══════ Tab Navigation ═══════ --}}
    <div class="flex border-b overflow-x-auto gap-0" style="border-color:var(--divider)">
        <button @click="activeTab = 'financial'" class="px-5 py-3 text-sm font-bold transition-colors whitespace-nowrap border-b-2" :style="activeTab === 'financial' ? 'color:#111827;border-bottom-color:#111827' : 'color:var(--text-secondary);border-bottom-color:transparent'">المالية</button>
        <button @click="activeTab = 'products'" class="px-5 py-3 text-sm font-bold transition-colors whitespace-nowrap border-b-2" :style="activeTab === 'products' ? 'color:#111827;border-bottom-color:#111827' : 'color:var(--text-secondary);border-bottom-color:transparent'">أرباح المنتجات</button>
        <button @click="activeTab = 'charts'" class="px-5 py-3 text-sm font-bold transition-colors whitespace-nowrap border-b-2" :style="activeTab === 'charts' ? 'color:#111827;border-bottom-color:#111827' : 'color:var(--text-secondary);border-bottom-color:transparent'">الرسوم البيانية</button>
    ...
    <div x-show="activeTab === 'financial'" x-cloak>
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
            ...
            <div class="stat-card">
                <div class="flex items-center gap-2 mb-2">
                    <div class="w-8 h-8 rounded-lg flex items-center justify-center" style="background:rgba(251,146,60,0.1)"><svg class="w-4 h-4" style="color:#fb923c" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M12 6v12m-3-2.818l.879.659c1.171.879 3.07.879 4.242 0 1.172-.879 1.172-2.303 0-3.182C13.536 12.219 12.768 12 12 12c-.725 0-1.45-.22-2.003-.659-1.106-.879-1.106-2.303 0-3.182s2.9-.879 4.006 0l.415.33M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg></div>
                            ...
                            <tr class="transition-colors" onmouseover="this.style.background=var(--hover-bg)" onmouseout="this.style.background='transparent'">
                                <td class="px-5 py-3">
                                    <div class="flex items-center gap-3">
                                        @if($p['image'])<img src="{{ asset('storage/'.$p['image']) }}" class="w-9 h-9 rounded-lg object-cover" style="border:1px solid var(--border)">@else<div class="w-9 h-9 rounded-lg flex items-center justify-center" style="background:var(--input-bg);border:1px solid var(--border)"><svg class="w-4 h-4" style="color:var(--text-tertiary)" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M20.25 7.5l-.625 10.632a2.25 2.25 0 01-2.247 2.118H6.622a2.25 2.25 0 01-2.247-2.118L3.75 7.5m6 4.125l2.25 2.25m0 0l2.25-2.25M12 13.875V7.5"/></svg></div>@endif
                ...
                <div class="space-y-4">
                    ...
                    <div class="border-t pt-4" style="border-color:var(--border)">
                        <div class="grid grid-cols-2 gap-4 text-center">
                            <div class="stat-card"><div class="text-lg font-extrabold" style="color:#4f8cff" x-text="selectedProduct?.units_sold"></div><div class="text-xs" style="color:var(--text-secondary)">وحدة مباعة</div></div>
                            <div class="stat-card"><div class="text-lg font-extrabold" style="color:#34d399" x-text="'+' + selectedProduct?.total_profit?.toLocaleString()"></div><div class="text-xs" style="color:var(--text-secondary)">إجمالي الربح (DA)</div></div>
                        </div>
                    </div>
                </div>
            </div>
    ...
    new Chart(monthlyCtx.getContext('2d'), {
        ...
        options: {
            ...
            scales: {
                ...
                y: { ticks: { color: '#8a92a6', font: { size: 10, family: 'Cairo' }, callback: v => v.toLocaleString() }, grid: { color: '#e5e7eb' } }
            }
        }
    });
}
...
if (statusCtx) {
    ...
    new Chart(statusCtx.getContext('2d'), {
        ...
        options: { responsive: true, maintainAspectRatio: false, cutout: '65%', plugins: { legend: { position: 'bottom', labels: { color: '#6b7280', font: { size: 11, family: 'Cairo' }, padding: 16 } } } }
    });
}
</script>
@endpush
