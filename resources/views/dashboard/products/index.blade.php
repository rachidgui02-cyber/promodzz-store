...
<div class="space-y-6" dir="rtl">
    <div class="flex items-center justify-between">
        ...
        <a href="{{ route('dashboard.products.create') }}" class="flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-bold transition-all hover:scale-[1.02]" style="background:#111827;color:#ffffff">
    ...
    <div class="stat-card rounded-2xl border p-5" style="border-color:var(--border)">
        <form action="{{ route('dashboard.products.index') }}" method="GET" class="flex flex-col md:flex-row gap-3">
            <div class="flex-1 relative">
                <input type="text" name="search" value="{{ request('search', '') }}" placeholder="بحث بالاسم، SKU..." class="w-full rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-1 focus:ring-accent-blue pr-10" style="background:var(--input-bg);border:1px solid var(--border);color:var(--text-primary)" placeholder="بحث...">
                <svg class="w-4 h-4 absolute right-3 top-1/2 -translate-y-1/2" style="color:var(--text-tertiary)" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/></svg>
            </div>
            <select name="category_id" class="rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-1 focus:ring-accent-blue" style="background:var(--input-bg);border:1px solid var(--border);color:var(--text-primary)">
                <option value="">جميع الفئات</option>
                @foreach($categories ?? [] as $category)
                    <option value="{{ $category->id }}" {{ request('category_id') == $category->id ? 'selected' : '' }}>{{ $category->name }}</option>
                @endforeach
            </select>
            <button type="submit" class="px-6 py-2.5 rounded-xl text-sm font-bold transition-all hover:scale-[1.02]" style="background:#111827;color:#ffffff">بحث</button>
        </form>
    </div>
            ...
            <table class="w-full text-sm">
                <thead>
                    <tr style="background:rgba(255,255,255,0.03)">
                        ...
                        <th class="text-right px-5 py-3 font-semibold text-xs" style="color:var(--text-secondary)">SKU</th>
                        <th class="text-right px-5 py-3 font-semibold text-xs" style="color:var(--text-secondary)">م. الشراء</th>
                        <th class="text-right px-5 py-3 font-semibold text-xs" style="color:var(--text-secondary)">م. البيع</th>
                        <th class="text-right px-5 py-3 font-semibold text-xs" style="color:var(--text-secondary)">هامش الربح</th>
                        <th class="text-right px-5 py-3 font-semibold text-xs" style="color:var(--text-secondary)">المخزون</th>
                ...
                <tbody class="divide-y" style="border-color:var(--border)">
                        ...
                        <tr class="transition-colors" onmouseover="this.style.background=var(--hover-bg)" onmouseout="this.style.background='transparent'">
                            <td class="px-5 py-3">
                                <div class="flex items-center gap-3">
                                    <div class="w-10 h-10 rounded-xl flex items-center justify-center shrink-0" style="background:var(--hover-bg);border:1px solid var(--border)">
                                        @if($product->image)<img src="{{ asset('storage/' . $product->image) }}" alt="" class="w-10 h-10 rounded-xl object-cover">@else<svg class="w-5 h-5" style="color:var(--text-tertiary)" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4"/></svg>@endif
                                    </div>
                                    <div>
                                        <p class="font-medium text-white text-sm">{{ $product->name }}</p>
                                        @if($product->category)<p class="text-xs" style="color:var(--text-secondary)">{{ $product->category->name }}</p>@endif
                                    </div>
                                </div>
                            </td>
                            <td class="px-5 py-3 font-mono text-xs" style="color:var(--text-secondary)">{{ $product->sku }}</td>
                            <td class="px-5 py-3" style="color:var(--text-secondary)">{{ number_format($product->buy_price, 2) }} DA</td>
                    ...
                    @empty
                        <tr><td colspan="8" class="px-5 py-10 text-center" style="color:var(--text-tertiary)">لا توجد منتجات</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
        @if($products ?? false)<div class="p-5 border-t" style="border-color:var(--border)">{{ $products->withQueryString()->links() }}</div>@endif
    </div>
</div>
@endsection
